...
def load_models(
    ...
    caption_model.load_state_dict(
        torch.load(caption_net_path, map_location=torch.device("cpu"))
    )
    ...
    if pseudo_inception:
        ...
        inception.load_state_dict(
            torch.load(inception_path, map_location=torch.device("cpu"))
        )
...
def image_process(img):
    ...
    img_for_net = resize(
        img, (299, 299)
    )  # .astype('float32') / 255.  ### this is done by resize
...
def generate_caption(
    ...
    caption_prefix=(DEFAULT_BOS_IDX,),
    end_token_idx=DEFAULT_EOS_IDX,
    temperature=1,
    sample=True,
    max_len=10,
):
    ...
    with torch.no_grad():
        ...
        vectors_8x8, vectors_neck, logits = inception(
            image[None]
        )  #### adding batch size, then channels, then h, w

        caption_prefix = list(caption_prefix)
        text_caption = []

        with torch.no_grad():
            # generating image description token by token
            # actually what is happening: on every step we append last predicted word and run the net from start with the same initial vector
            for _ in range(max_len):
                # 1. make caption_prefix as a matrix
                # 2. getting logits from rnn, feeding it vectors_neck and matrix from step #1
                # 3. transforming logits into probabilities through softmax
                # 4. sampling (greedy or not) next token using distribution above
                # 5. appending new token into caption_prefix
                # 6. stopping if end of sentence was generated

                captions_ix_inp = torch.tensor(
                    caption_prefix, dtype=torch.long
                ).unsqueeze(0)
                logits_for_next = caption_model.forward(vectors_neck, captions_ix_inp)
                next_token_distr = F.softmax(
                    logits_for_next[0, -1, :] / temperature, dim=-1
                ).numpy()  ### fetching only last prediction
                next_token_idx_hard = next_token_distr.argmax(axis=-1)

                # fixing prediction due to possibility of special tokens
                ...
                next_token_distr[~zeroing_mask] += relocate_proba_sum / np.sum(
                    ~zeroing_mask
                )  #### renormalizing probabilities

                next_token_idx_sampled = np.random.choice(
                    np.arange(len(next_token_distr)), p=next_token_distr
                )
